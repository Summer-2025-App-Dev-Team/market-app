...
export default function Service(props) {
    ...
    if (props.availableUntil) {
        ...
        date = new Date(
            props.availableUntil.seconds * 1000 +
                props.availableUntil.nanoseconds / 1e6
        );
    ...
    return (
        <div
            className={`${styles["service-wrapper"]} ${props.preview ? styles.preview : ""} ${props.status === "unavailable" ? styles.unavailable : ""}`}
            onClick={() => {
                !props.preview && props.status === "available"
        ...
        >
            ...
            {loading ? (
                <div className={styles["loading-background"]}>Loading...</div>
            ) : (
                <>
                    <div className={styles["item-info"]}>
                        <span className={styles.name}>
                            {props.name ? props.name : "No name set yet"}
                        </span>
                    ...
                    <span
                        className={`${styles.price} ${price.toLowerCase() == "free" ? styles.free : ""}`}
                    >
                        {price}
                    </span>
