...
export default function ChatRoomPreview({ chatId }) {
  ...
  useEffect(() => {
    //fetch participants & messages from db
    const usersRef = ref(realtimedb, `chats/${chatId}`);
    const messagesRef = ref(realtimedb, `chats/${chatId}/chats`);
    onValue(usersRef, (snapshot) => {
      let userId;
      let sellerId;
      const data = snapshot.val(); //gets the value of the chat snapshot
      console.log(data);
      let sellerSnap = null;
      async function fetchUsers() {
        if (data.user1 != user.uid) {
          sellerId = data.user1;
          userId = data.user2;
          const sellerDocRef = doc(db, "userListings", sellerId);
          sellerSnap = await getDoc(sellerDocRef);
        } else {
          sellerId = data.user2;
          userId = data.user1;
          const sellerDocRef = doc(db, "userListings", sellerId);
      ...
      fetchUsers(); //fetch the users from firestore
    });
  }, [chatId]);
  //   useEffect(() => {
